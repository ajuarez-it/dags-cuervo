...
with DAG(
    ...
    with TaskGroup("Bronze", default_args={'pool': 'emetrix'}) as TG_bronze:
        ...
        for idx, source in enumerate(bronze_sources, start=1):
            trigger_cloud_run_job_freshness_bronze_areas_areas_nielsen= CloudRunExecuteJobOperator(
                task_id=f"trigger_cloud_run_job_freshness_bronze_areas_areas_nielsen_{idx:02d}",
                overrides={
                    "container_overrides": [
                        {
                            "args": [
            ...
            if prev_task:
                prev_task >> trigger_cloud_run_job_freshness_bronze_areas_areas_nielsen
            prev_task = trigger_cloud_run_job_freshness_bronze_areas_areas_nielsen

    with TaskGroup("Silver", default_args={'pool': 'emetrix'}) as TG_silver:
        trigger_cloud_run_job_build_silver_areas_nielsen = CloudRunExecuteJobOperator(
            task_id="trigger_cloud_run_job_build_silver_areas_nielsen",
            overrides={
                "container_overrides": [
                    {
                        "args": [
                            "build",
                            "--select",
                            "staging.requester.stg_d_areas_nielsen"
                        ],
                    }
                ],
            },
    ...
    with TaskGroup("Gold", default_args={'pool': 'emetrix'}) as TG_gold:
        trigger_cloud_run_job_build_gold_billing = CloudRunExecuteJobOperator(
            task_id="billing_gold",
            overrides={
                "container_overrides": [
                    {
                        "args": [
                            "build",
                            "--select",
                            "dbt_cuervo.GLD_GLOBAL_MASTER_REPORT.c_areas_nielsen"
                        ],
                    }
                ],
            },
