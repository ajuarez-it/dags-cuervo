...
with DAG(
    ...
    catchup=False,
    tags=["CATALOG", "CALENDAR", "SILVER", "GOLD"],
    ...
    trigger_cloud_run_job_for_gold_calendar = CloudRunExecuteJobOperator(
        ...
        overrides={
            "container_overrides": [
                {
                    "name": JOB_NAME,
                    "args": ["run", "--select", "dbt_cuervo.marts.commercial.f_mcc_calendar"],
                }
            ]
        },
    ...
    trigger_cloud_run_job_test_bronze_calendar = CloudRunExecuteJobOperator(
        task_id="trigger_cloud_run_job_test_bronze_calendar",
        project_id=GCP_PROJECT_ID,
        region=GCP_REGION,
        job_name=JOB_NAME,
        gcp_conn_id=GCP_CONN_ID,
    ...
    trigger_cloud_run_job_test_gold_calendar = CloudRunExecuteJobOperator(
        ...
        overrides={
            "container_overrides": [
                {
                    "name": JOB_NAME,
                    "args": ["test", "--select", "dbt_cuervo.marts.commercial.f_mcc_calendar"],
                }
            ]
        },
...
(
    start
    >> trigger_cloud_run_job_test_bronze_calendar
