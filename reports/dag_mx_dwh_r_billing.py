...
with DAG(
    ...
    catchup=False,
    tags=["MCC", "BILLING", "SILVER", "GOLD"],
    description="A DAG to trigger billing Workflow with BigQuery, and Cloud Run jobs.",
) as dag:
    ...
    with TaskGroup("Bronze", default_args={'pool': 'emetrix'}) as TG_bronze:
        ...
        for idx, source in enumerate(bronze_sources, start=1):
            trigger_cloud_run_job_freshness_bronze_billing= CloudRunExecuteJobOperator(
                task_id=f"trigger_cloud_run_job_freshness_bronze_billing_{idx:02d}",
                overrides={
                    "container_overrides": [
                        {
                            "args": [
            ...
            if prev_task:
                prev_task >> trigger_cloud_run_job_freshness_bronze_billing
            prev_task = trigger_cloud_run_job_freshness_bronze_billing

    with TaskGroup("Silver", default_args={'pool': 'emetrix'}) as TG_silver:
        trigger_cloud_run_job_build_silver_billing = CloudRunExecuteJobOperator(
            task_id="trigger_cloud_run_job_build_silver_billing",
            overrides={
                "container_overrides": [
                    {
                        "args": [
